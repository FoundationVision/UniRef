...
if __name__ == "__main__":
    for dataset in ["refcoco-unc", "refcocog-umd", "refcocoplus-unc"]:
        json_path = "datasets/coco2014/annotations/%s/instances.json" % dataset
        os.system("python3 conversion/convert_ref2coco.py --src_json %s --des_json %s" %(json_path, json_path))
    # merge train split
    merged_dir = "datasets/coco2014/annotations/refcoco-mixed"
    if not os.path.exists(merged_dir):
        os.makedirs(merged_dir)
    merged_json = "datasets/coco2014/annotations/refcoco-mixed/instances_train.json"
