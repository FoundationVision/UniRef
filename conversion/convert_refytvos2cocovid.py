"""
Each object is a sample, maybe with multiple expressions.
"""
import json
import argparse
import os
...
if __name__ == "__main__":
    ...
    for split in splits:
        ...
        for vid_idx, vid in enumerate(videos):
            ...
            # parse expressions in a video, a object may be with multiple expressions
            data_dict = {}
            ...
            for frame_idx in range(len(frames)):
                ...
                for obj_id in metas_vid.keys():
                    ...
                    if (mask_cur > 0).any():
                        ...
                        vid_obj_dict[obj_id]["bboxes"].append(None)
                        vid_obj_dict[obj_id]["segmentations"].append(None)
                        vid_obj_dict[obj_id]["areas"].append(None)
                        vid_info_dict[obj_id]["file_names"].append(os.path.join(vid, masks[frame_idx].replace(".png", ".jpg")))            
            
            # save to annotations, every object is a sample
            for obj_id in metas_vid.keys():
