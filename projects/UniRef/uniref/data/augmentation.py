...
class ResizeLongestEdge(T.Augmentation):
    """
    Scale the longer edge to the given size, with a limit of `max_size` on the longer edge.
    If `max_size` is reached, then downscale so that the longer edge does not exceed max_size.
    """

    def __init__(
        self, long_edge_length, max_size=sys.maxsize, sample_style="range", interp=Image.BILINEAR, clip_frame_cnt=1
    ):
        """
        Args:
        ...
        if self.is_range:
            assert len(long_edge_length) == 2, (
                "long_edge_length must be two values using 'range' sample style."
    ...
    def get_transform(self, image):
        if self._cnt % self.clip_frame_cnt == 0:
            if self.is_range:
                self.size = np.random.randint(self.long_edge_length[0], self.long_edge_length[1] + 1)
            else:
                self.size = np.random.choice(self.long_edge_length)
            if self.size == 0:
                return NoOpTransform()
...
def build_augmentation_sam(cfg, is_train, min_size_train, max_size_train):
    logger = logging.getLogger(__name__)
    aug_list = []
    if is_train:
        # Crop
        ...
        sample_style = cfg.INPUT.MIN_SIZE_TRAIN_SAMPLING
        ms_clip_frame_cnt = cfg.INPUT.SAMPLING_FRAME_NUM if "by_clip" in cfg.INPUT.MIN_SIZE_TRAIN_SAMPLING else 1
        aug_list.append(ResizeLongestEdge(1024, sample_style=sample_style, clip_frame_cnt=ms_clip_frame_cnt))
        ...
        # Resize
        sample_style = "choice"
        aug_list.append(T.ResizeLongestEdge(1024, sample_style=sample_style))
        return aug_list
